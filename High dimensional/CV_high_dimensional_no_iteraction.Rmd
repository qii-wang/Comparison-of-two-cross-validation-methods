...
for(i in 1:nrow(paras)){
  cat("\n", i, "\n")
  num_folds = paras$num_folds[i]
  res = foreach(iter=1:n_iter, .combine = 'rbind', .packages = c("caret", "pROC", "tidyverse", "xgboost")) %dopar%{
    setTxtProgressBar(pb, iter)
    sim_data = readRDS(paste0("./Simulated data/highD_data", iter, ".rds"))
    ext_data = readRDS(paste0("./Simulated external data/external_highD_data", iter, ".rds"))
    N = nrow(sim_data)
    ...
    x_train = sim_data[, -ncol(sim_data)]
    y_train = sim_data$case
    ...
    xgboost_para_tune = expand.grid(max_depth = c(3,5,7),
                                    test_auc = 0,
                                    round = 0)
    scale_pos_weight = sum(1-sim_data$case) / sum(sim_data$case)
    for(tune_iter in 1:nrow(xgboost_para_tune)){
      params = list(max_depth = xgboost_para_tune$max_depth[tune_iter])
      ...
      xgboost_para_tune$test_auc[tune_iter] = bst$evaluation_log$test_auc_mean[bst$best_iteration]
      xgboost_para_tune$round[tune_iter] = bst$best_iteration
    }
    
    best_para_id = which.max(xgboost_para_tune$test_auc)
    best_nrounds = xgboost_para_tune$round[best_para_id]
    best_maxdepth = xgboost_para_tune$max_depth[best_para_id]
    
    full_data_model = xgboost(data = xgb_df,
                          nrounds = best_nrounds,
                          max.depth = best_maxdepth,
                          objective = "binary:logistic",
                          verbose = FALSE,
                          scale_pos_weight = scale_pos_weight)
    
    
    ext_data$Y_pred = predict(full_data_model, newdata = model.matrix(~.-1, ext_data[,-ncol(ext_data)]))
    ...
    for(cv_iter in 1:num_folds){
      ...
      cv_x_train = cv_train_data[, -ncol(cv_train_data)]
      cv_y_train = cv_train_data$case
      cv_x_train_df = model.matrix(~.-1, data = cv_x_train)
      cv_xgb_df = xgb.DMatrix(data = cv_x_train_df, label = cv_y_train)
      cv_model = xgboost(data = cv_xgb_df,
                         nrounds = best_nrounds,
                         max.depth = best_maxdepth,
                         objective = "binary:logistic",
                         verbose = FALSE,
      ...
      cv_Y_ped = predict(cv_model, newdata = model.matrix(~.-1, cv_test_data[,-ncol(cv_test_data)]))
      test_Y_pred = c(test_Y_pred, cv_Y_ped)
      cv_test_data$Y_pred = cv_Y_ped
      suppressMessages(auc_byAvg_obj_W0 <- with(cv_test_data[which(cv_test_data_W == 0), ], auc(case, Y_pred)))
      suppressMessages(auc_byAvg_obj_W1 <- with(cv_test_data[which(cv_test_data_W == 1), ], auc(case, Y_pred)))
...
out_AUC %>% write.csv("./Simulation results/Full stratification/XGBoost/AUC.csv", row.names = FALSE)
out_EOR %>% write.csv("./Simulation results/Full stratification/XGBoost/EOR.csv", row.names = FALSE)
out_cali_intc %>% write.csv("./Simulation results/Full stratification/XGBoost/Cali_intercept.csv", row.names = FALSE)
out_cali_slp %>% write.csv("./Simulation results/Full stratification/XGBoost/Cali_slope.csv", row.names = FALSE)
```